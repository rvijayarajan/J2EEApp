...
import org.appfuse.service.LcxsManager;
...
import org.appfuse.model.Lcxs;

import org.appfuse.webapp.form.LcxsForm;

/**
 * Action class to handle CRUD on a Lcxs object
 *
 * @struts.action name="lcxsForm" path="/lcxss" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="lcxsForm" path="/editLcxs" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="lcxsForm" path="/saveLcxs" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/lcxsForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/lcxsList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/lcxsFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/lcxsView.jsp"
 */
public final class LcxsAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		LcxsForm lcxsForm = (LcxsForm) form;

		resetLcxs(lcxsForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		LcxsManager mgr = (LcxsManager) getBean("lcxsManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, LcxsManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			LcxsManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetLcxs((LcxsForm) form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetLcxs((LcxsForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeLcxs(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.lcxs.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "lcxs.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		LcxsForm lcxsForm = new LcxsForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String lcxslist = Constants.LCXS_LIST;
		String lcxsid = Constants.LCXS_ID;
		return do_find(mapping, form, request, lcxslist, lcxsid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String lcxslist = Constants.LCXS_LIST;
		String lcxsid = Constants.LCXS_ID;
		return do_Search(mapping, request, lcxslist, lcxsid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		LcxsForm lcxsForm = (LcxsForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(lcxsForm.getId()) || lcxsForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetLcxs(lcxsForm); //将lcxsForm清空
				}
			} else {
				Lcxs lcxs; //按照id进行查询
				try {
					LcxsManager mgr = (LcxsManager) getBean("lcxsManager");
					lcxs = mgr.getLcxs(id);
					lcxsForm = (LcxsForm) convert(lcxs);
					request.setAttribute(Constants.PID, lcxs.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, lcxsForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String lcxslist, String lcxsid)
			throws BeansException {
		String type = getType(request);
		...
		LcxsForm lcxs;
		try {
			lcxs = (LcxsForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			lcxs = null;
		}
		...
		map = SearchMap.getLcxsMap(request, lcxs); //得到map
		...
		ValueList valueList = vlh.getValueList(lcxslist, info);

		vlHelper.backupAndSet(request, valueList, lcxslist, lcxsid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		lcxsForm.setXgr(getXgr(request));
		lcxsForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(lcxsForm.getXgsj()));//打印修改时间。

		LcxsManager mgr = (LcxsManager) getBean("lcxsManager");
		Lcxs lcxs = (Lcxs) convert(lcxsForm);
		...
		try {
			String id = mgr.saveLcxs(lcxs); //保存信息
			lcxsForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "lcxs.saved");
			...
			saveMsg(request, messages, "error.lcxs.saved");
			request.setAttribute("pid", lcxsForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetLcxs(LcxsForm lcxsForm) {
		Lcxs lcxs = new Lcxs();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(lcxsForm, lcxs);
		} catch (IllegalAccessException e1) {
			log.debug("resetLcxs 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetLcxs 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		LcxsForm lcxs = (LcxsForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getLcxsMap(request, lcxs); //得到map //getMap(request, lcxs);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(lcxslist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, lcxslist, lcxsid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "lcxs查询结果.xls"; //注意：要修改该文件名
		String listname = lcxslist;
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			LcxsManager mgr = (LcxsManager) getBean("lcxsManager");
			Lcxs lcxs = mgr.getLcxs(id); //按照id进行查询
			lcxsForm = (LcxsForm) convert(lcxs);
			request.setAttribute(Constants.PID, lcxs.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, lcxsForm);
