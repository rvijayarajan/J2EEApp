...
public final class TjbzsyAction extends BaseAction {
    ...
    throws Exception {
	 ...
	 resetTjbzsy(tjbzsyForm); //FormBean复位
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetTjbzsy((TjbzsyForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeTjbzsy(id);  //删除记录
			} catch (RuntimeException e) {
	...
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String tjbzsylist, String tjbzsyid) throws BeansException {
		...
		map = SearchMap.getTjbzsyMap(request, tjbzsy); //得到map
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			TjbzsyManager mgr = (TjbzsyManager) getBean("tjbzsyManager");
            try {
                getTjbzsy(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.tjbzsy.selected", id);
    ...
    throws Exception {
		...
		tjbzsyForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(tjbzsyForm.getXgsj()));//打印修改时间。
	...
	private void resetTjbzsy(TjbzsyForm tjbzsyForm) {
		...
		try {
			...
			log.debug("resetTjbzsy 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetTjbzsy 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String tjbzsylist, String tjbzsyid) throws BeansException {
		...
		map = SearchMap.getTjbzsyMap(request, tjbzsy); //得到map //getMap(request, tjbzsy); //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "tjbzsy查询结果.xls"; //注意：要修改该文件名
