...
import org.appfuse.service.FbManager;
...
import org.appfuse.model.Fb;

import org.appfuse.webapp.form.FbForm;

/**
 * Action class to handle CRUD on a Fb object
 *
 * @struts.action name="fbForm" path="/fbs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="fbForm" path="/editFb" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="fbForm" path="/saveFb" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/fbForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/fbList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/fbFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/fbView.jsp"
 */
public final class FbAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		FbForm fbForm = (FbForm) form;

		resetFb(fbForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		FbManager mgr = (FbManager) getBean("fbManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, FbManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			FbManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetFb((FbForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeFb(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.fb.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "fb.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String fblist = Constants.FB_LIST;
		String fbid = Constants.FB_ID;
		return do_find(mapping, form, request, fblist, fbid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String fblist = Constants.FB_LIST;
		String fbid = Constants.FB_ID;
		return do_Search(mapping, request, fblist, fbid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		FbForm fbForm = (FbForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(fbForm.getId()) || fbForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					FbManager mgr = (FbManager) getBean("fbManager");
					fb = mgr.getFb(id);
					fbForm = (FbForm) convert(fb);
					request.setAttribute(Constants.PID, fb.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, fbForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String fblist, String fbid)
			throws BeansException {
		String type = getType(request);
		...
		FbForm fb;
		try {
			fb = (FbForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			fb = null;
		}
		...
		map = SearchMap.getFbMap(request, fb); //得到map
		...
		ValueList valueList = vlh.getValueList(fblist, info);

		vlHelper.backupAndSet(request, valueList, fblist, fbid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		fbForm.setXgr(getXgr(request));
		fbForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(fbForm.getXgsj()));//打印修改时间。

		FbManager mgr = (FbManager) getBean("fbManager");
		Fb fb = (Fb) convert(fbForm);
		...
		try {
			String id = mgr.saveFb(fb); //保存信息
			fbForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "fb.saved");
			...
			saveMsg(request, messages, "error.fb.saved");
			request.setAttribute("pid", fbForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetFb(FbForm fbForm) {
		Fb fb = new Fb();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(fbForm, fb);
		} catch (IllegalAccessException e1) {
			log.debug("resetFb 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetFb 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		FbForm fb = (FbForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getFbMap(request, fb); //得到map //getMap(request, fb);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(fblist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, fblist, fbid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "fb查询结果.xls"; //注意：要修改该文件名
		String listname = fblist;
