...
import org.appfuse.service.BxManager;
...
import org.appfuse.model.Bx;

import org.appfuse.webapp.form.BxForm;

/**
 * Action class to handle CRUD on a Bx object
 *
 * @struts.action name="bxForm" path="/bxs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="bxForm" path="/editBx" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="bxForm" path="/saveBx" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/bxForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/bxList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/bxFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/bxView.jsp"
 */
public final class BxAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		BxForm bxForm = (BxForm) form;

		resetBx(bxForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		BxManager mgr = (BxManager) getBean("bxManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, BxManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			BxManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetBx((BxForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeBx(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.bx.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "bx.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String bxlist = Constants.BX_LIST;
		String bxid = Constants.BX_ID;
		return do_find(mapping, form, request, bxlist, bxid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String bxlist = Constants.BX_LIST;
		String bxid = Constants.BX_ID;
		return do_Search(mapping, request, bxlist, bxid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		BxForm bxForm = (BxForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(bxForm.getId()) || bxForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					BxManager mgr = (BxManager) getBean("bxManager");
					bx = mgr.getBx(id);
					bxForm = (BxForm) convert(bx);
					request.setAttribute(Constants.PID, bx.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, bxForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String bxlist, String bxid)
			throws BeansException {
		String type = getType(request);
		...
		BxForm bx;
		try {
			bx = (BxForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			bx = null;
		}
		...
		map = SearchMap.getBxMap(request, bx); //得到map
		...
		ValueList valueList = vlh.getValueList(bxlist, info);

		vlHelper.backupAndSet(request, valueList, bxlist, bxid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		bxForm.setXgr(getXgr(request));
		bxForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(bxForm.getXgsj()));//打印修改时间。

		BxManager mgr = (BxManager) getBean("bxManager");
		Bx bx = (Bx) convert(bxForm);
		...
		try {
			String id = mgr.saveBx(bx); //保存信息
			bxForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "bx.saved");
			...
			saveMsg(request, messages, "error.bx.saved");
			request.setAttribute("pid", bxForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetBx(BxForm bxForm) {
		Bx bx = new Bx();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(bxForm, bx);
		} catch (IllegalAccessException e1) {
			log.debug("resetBx 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetBx 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		BxForm bx = (BxForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getBxMap(request, bx); //得到map //getMap(request, bx);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(bxlist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, bxlist, bxid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "bx查询结果.xls"; //注意：要修改该文件名
