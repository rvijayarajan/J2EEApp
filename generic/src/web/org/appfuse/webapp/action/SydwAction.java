...
import org.appfuse.service.SydwManager;
...
import org.appfuse.model.Sydw;

import org.appfuse.webapp.form.SydwForm;

/**
 * Action class to handle CRUD on a Sydw object
 *
 * @struts.action name="sydwForm" path="/sydws" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="sydwForm" path="/editSydw" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="sydwForm" path="/saveSydw" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/sydwForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/sydwList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/sydwFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/sydwView.jsp"
 */
public final class SydwAction extends BaseAction {

    public ActionForward cancel(ActionMapping mapping, ActionForm form,
                                HttpServletRequest request,
                                HttpServletResponse response)
    throws Exception {
	 SydwForm sydwForm = (SydwForm) form;

	 resetSydw(sydwForm); //FormBean复位
    ...
    throws Exception {
		...
		SydwManager mgr = (SydwManager) getBean("sydwManager");
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			...
			SydwManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetSydw((SydwForm)form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			SydwManager mgr, ActionMessages messages,
			String type) throws Exception {
		HttpSession session = request.getSession(); //得到Session，从Session中查找是否有未处理的id
		String id = null;
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetSydw((SydwForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeSydw(id);  //删除记录
			} catch (RuntimeException e) {
				getError(session, e, "error.sydw.deleted", id);
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "sydw.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		SydwForm sydwForm = new SydwForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String sydwlist = Constants.SYDW_LIST;
		String sydwid = Constants.SYDW_ID;
		return do_find(mapping, form, request, sydwlist, sydwid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String sydwlist = Constants.SYDW_LIST;
		String sydwid = Constants.SYDW_ID;
		return do_Search(mapping, request, sydwlist, sydwid);
	}
    ...
    throws Exception {
		...
		boolean isNew = ("".equals(sydwForm.getId()) || sydwForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			SydwManager mgr = (SydwManager) getBean("sydwManager");
			 try {
                getSydw(mapping, request, id, mgr);
            } catch (RuntimeException e) {
                getError(session, e, "error.sydw.selected", id);
     ...
     * @throws Exception
     */
    private void getSydw(ActionMapping mapping, HttpServletRequest request, String id, SydwManager mgr) throws Exception {
        SydwForm sydwForm;
        Sydw sydw;
        sydw = mgr.getSydw(id);
        sydwForm = (SydwForm) convert(sydw);
        updateFormBean(mapping, request, sydwForm);
	 ...
	 */
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String sydwlist, String sydwid) throws BeansException {
		String type = getType(request);
		...
		SydwForm sydw ;
		try {
			sydw = (SydwForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			 sydw = null;
		}
		...
		map = SearchMap.getSydwMap(request, sydw); //得到map
		...
		ValueList valueList = vlh.getValueList(sydwlist, info);
		vlHelper.backupAndSet(request, valueList, sydwlist, sydwid);
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			SydwManager mgr = (SydwManager) getBean("sydwManager");
            try {
                getSydw(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.sydw.selected", id);
    ...
    throws Exception {
		...
		//得到修改人
		sydwForm.setXgr(getXgr(request));
		sydwForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(sydwForm.getXgsj()));//打印修改时间。

		SydwManager mgr = (SydwManager) getBean("sydwManager");
		Sydw sydw = (Sydw) convert(sydwForm);
		...
		try {
			id = mgr.saveSydw(sydw); //保存信息

			sydwForm.setId(id);

			saveMsg(request, messages, "sydw.saved");

			return mapping.findForward("view");
		} catch (RuntimeException e) {
			e.printStackTrace();
			saveMsg(request, messages, "error.sydw.saved");
	...
	public ActionForward batch_ChangeID(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            getError(session, e1, "error.sydw.selected", id);
        }
	 ...
	 * 
	 */
	private void resetSydw(SydwForm sydwForm) {
		Sydw sydw = new Sydw();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(sydwForm, sydw);
		} catch (IllegalAccessException e1) {
			log.debug("resetSydw 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetSydw 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	 ...
	 * @throws BeansException
	 */
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String sydwlist, String sydwid) throws BeansException {
		HttpSession session = request.getSession();
		//清除Session中存有的信息
		session.removeAttribute(Constants.SEL_IDS);
		String type = getType(request);

		SydwForm sydw = (SydwForm) form;
		//备份与字段相关的form信息
		session.setAttribute("form",sydw);
		...
		map = SearchMap.getSydwMap(request, sydw); //得到map //getMap(request, sydw); //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(sydwlist, new ValueListInfo(
				map));
		vlHelper.backupAndSet(request, valueList, sydwlist, sydwid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "sydw查询结果.xls"; //注意：要修改该文件名
		String listname = sydwlist;
