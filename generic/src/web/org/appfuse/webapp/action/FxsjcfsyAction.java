...
public final class FxsjcfsyAction extends BaseAction {
    ...
    throws Exception {
	 ...
	 resetFxsjcfsy(fxsjcfsyForm); //FormBean复位
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetFxsjcfsy((FxsjcfsyForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeFxsjcfsy(id);  //删除记录
			} catch (RuntimeException e) {
	...
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String fxsjcfsylist, String fxsjcfsyid) throws BeansException {
		...
		map = SearchMap.getFxsjcfsyMap(request, fxsjcfsy); //得到map
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			FxsjcfsyManager mgr = (FxsjcfsyManager) getBean("fxsjcfsyManager");
            try {
                getFxsjcfsy(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.fxsjcfsy.selected", id);
    ...
    throws Exception {
		...
		fxsjcfsyForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(fxsjcfsyForm.getXgsj()));//打印修改时间。
		...
		try {
			id = mgr.saveFxsjcfsy(fxsjcfsy); //保存信息
	...
	private void resetFxsjcfsy(FxsjcfsyForm fxsjcfsyForm) {
		...
		try {
			...
			log.debug("resetFxsjcfsy 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetFxsjcfsy 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String fxsjcfsylist, String fxsjcfsyid) throws BeansException {
		...
		map = SearchMap.getFxsjcfsyMap(request, fxsjcfsy); //得到map //getMap(request, fxsjcfsy); //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "fxsjcfsy查询结果.xls"; //注意：要修改该文件名
