...
public final class TfsjxsjlAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
		...
		resetTfsjxsjl(tfsjxsjlForm); //FormBean复位
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetTfsjxsjl((TfsjxsjlForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetTfsjxsjl(tfsjxsjlForm); //将tfsjxsjlForm清空
				}
			} else {
				Tfsjxsjl tfsjxsjl; //按照id进行查询
				try {
					TfsjxsjlManager mgr = (TfsjxsjlManager) getBean("tfsjxsjlManager");
					tfsjxsjl = mgr.getTfsjxsjl(id);
					tfsjxsjlForm = (TfsjxsjlForm) convert(tfsjxsjl);
					request.setAttribute(Constants.PID, tfsjxsjl.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
	...
	public ActionForward do_Search(ActionMapping mapping,
		...
		map = SearchMap.getTfsjxsjlMap(request, tfsjxsjl); //得到map
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		tfsjxsjlForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(tfsjxsjlForm.getXgsj()));//打印修改时间。
		...
		try {
			String id = mgr.saveTfsjxsjl(tfsjxsjl); //保存信息
			...
			request.setAttribute("pid", tfsjxsjlForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	...
	private void resetTfsjxsjl(TfsjxsjlForm tfsjxsjlForm) {
		...
		try {
			...
			log.debug("resetTfsjxsjl 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetTfsjxsjl 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		map = SearchMap.getTfsjxsjlMap(request, tfsjxsjl); //得到map //getMap(request, tfsjxsjl);
		// //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "tfsjxsjl查询结果.xls"; //注意：要修改该文件名
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			TfsjxsjlManager mgr = (TfsjxsjlManager) getBean("tfsjxsjlManager");
			Tfsjxsjl tfsjxsjl = mgr.getTfsjxsjl(id); //按照id进行查询
			tfsjxsjlForm = (TfsjxsjlForm) convert(tfsjxsjl);
			request.setAttribute(Constants.PID, tfsjxsjl.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, tfsjxsjlForm);
