...
import org.appfuse.service.JcManager;
...
import org.appfuse.model.Jc;

import org.appfuse.webapp.form.JcForm;

/**
 * Action class to handle CRUD on a Jc object
 *
 * @struts.action name="jcForm" path="/jcs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="jcForm" path="/editJc" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="jcForm" path="/saveJc" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/jcForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/jcList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/jcFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/jcView.jsp"
 */
public final class JcAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JcForm jcForm = (JcForm) form;

		resetJc(jcForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		JcManager mgr = (JcManager) getBean("jcManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, JcManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			JcManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJc((JcForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeJc(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.jc.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "jc.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String jclist = Constants.JC_LIST;
		String jcid = Constants.JC_ID;
		return do_find(mapping, form, request, jclist, jcid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String jclist = Constants.JC_LIST;
		String jcid = Constants.JC_ID;
		return do_Search(mapping, request, jclist, jcid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		JcForm jcForm = (JcForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(jcForm.getId()) || jcForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					JcManager mgr = (JcManager) getBean("jcManager");
					jc = mgr.getJc(id);
					jcForm = (JcForm) convert(jc);
					request.setAttribute(Constants.PID, jc.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, jcForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String jclist, String jcid)
			throws BeansException {
		String type = getType(request);
		...
		JcForm jc;
		try {
			jc = (JcForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			jc = null;
		}
		...
		map = SearchMap.getJcMap(request, jc); //得到map
		...
		ValueList valueList = vlh.getValueList(jclist, info);

		vlHelper.backupAndSet(request, valueList, jclist, jcid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		jcForm.setXgr(getXgr(request));
		jcForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(jcForm.getXgsj()));//打印修改时间。

		JcManager mgr = (JcManager) getBean("jcManager");
		Jc jc = (Jc) convert(jcForm);
		...
		try {
			String id = mgr.saveJc(jc); //保存信息
			jcForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "jc.saved");
			...
			saveMsg(request, messages, "error.jc.saved");
			request.setAttribute("pid", jcForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetJc(JcForm jcForm) {
		Jc jc = new Jc();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(jcForm, jc);
		} catch (IllegalAccessException e1) {
			log.debug("resetJc 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetJc 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		JcForm jc = (JcForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getJcMap(request, jc); //得到map //getMap(request, jc);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(jclist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, jclist, jcid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "jc查询结果.xls"; //注意：要修改该文件名
		String listname = jclist;
