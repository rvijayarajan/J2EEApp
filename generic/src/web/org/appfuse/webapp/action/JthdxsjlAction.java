...
import org.appfuse.service.JthdxsjlManager;
...
import org.appfuse.model.Jthdxsjl;

import org.appfuse.webapp.form.JthdxsjlForm;

/**
 * Action class to handle CRUD on a Jthdxsjl object
 *
 * @struts.action name="jthdxsjlForm" path="/jthdxsjls" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="jthdxsjlForm" path="/editJthdxsjl" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="jthdxsjlForm" path="/saveJthdxsjl" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/jthdxsjlForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/jthdxsjlList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/jthdxsjlFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/jthdxsjlView.jsp"
 */
public final class JthdxsjlAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JthdxsjlForm jthdxsjlForm = (JthdxsjlForm) form;

		resetJthdxsjl(jthdxsjlForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		JthdxsjlManager mgr = (JthdxsjlManager) getBean("jthdxsjlManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, JthdxsjlManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			JthdxsjlManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJthdxsjl((JthdxsjlForm) form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJthdxsjl((JthdxsjlForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeJthdxsjl(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.jthdxsjl.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "jthdxsjl.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JthdxsjlForm jthdxsjlForm = new JthdxsjlForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String jthdxsjllist = Constants.JTHDXSJL_LIST;
		String jthdxsjlid = Constants.JTHDXSJL_ID;
		return do_find(mapping, form, request, jthdxsjllist, jthdxsjlid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String jthdxsjllist = Constants.JTHDXSJL_LIST;
		String jthdxsjlid = Constants.JTHDXSJL_ID;
		return do_Search(mapping, request, jthdxsjllist, jthdxsjlid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		JthdxsjlForm jthdxsjlForm = (JthdxsjlForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(jthdxsjlForm.getId()) || jthdxsjlForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetJthdxsjl(jthdxsjlForm); //将jthdxsjlForm清空
				}
			} else {
				Jthdxsjl jthdxsjl; //按照id进行查询
				try {
					JthdxsjlManager mgr = (JthdxsjlManager) getBean("jthdxsjlManager");
					jthdxsjl = mgr.getJthdxsjl(id);
					jthdxsjlForm = (JthdxsjlForm) convert(jthdxsjl);
					request.setAttribute(Constants.PID, jthdxsjl.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, jthdxsjlForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String jthdxsjllist, String jthdxsjlid)
			throws BeansException {
		String type = getType(request);
		...
		//得到查询时的form信息
		JthdxsjlForm jthdxsjl;
		try {
			jthdxsjl = (JthdxsjlForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			jthdxsjl = null;
		}
		...
		ValueListInfo oldInfo = vlHelper.getValueListInfo(request,
				Constants.JTHDXSJL_ID);
		//获取从第几页跳转来的
		int pageno = oldInfo.getPagingPage();

		map = SearchMap.getJthdxsjlMap(request, jthdxsjl); //得到map
		...
		ValueList valueList = vlh.getValueList(jthdxsjllist, info);

		vlHelper.backupAndSet(request, valueList, jthdxsjllist, jthdxsjlid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		jthdxsjlForm.setXgr(getXgr(request));
		jthdxsjlForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(jthdxsjlForm.getXgsj()));//打印修改时间。

		JthdxsjlManager mgr = (JthdxsjlManager) getBean("jthdxsjlManager");
		Jthdxsjl jthdxsjl = (Jthdxsjl) convert(jthdxsjlForm);
		...
		try {
			String id = mgr.saveJthdxsjl(jthdxsjl); //保存信息
			jthdxsjlForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "jthdxsjl.saved");
			...
			//					"error.jthdxsjl.saved"));
			//			saveMessages(session, messages);

			saveMsg(request, messages, "error.jthdxsjl.saved");
			request.setAttribute("pid", jthdxsjlForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetJthdxsjl(JthdxsjlForm jthdxsjlForm) {
		Jthdxsjl jthdxsjl = new Jthdxsjl();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(jthdxsjlForm, jthdxsjl);
		} catch (IllegalAccessException e1) {
			log.debug("resetJthdxsjl 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetJthdxsjl 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		JthdxsjlForm jthdxsjl = (JthdxsjlForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getJthdxsjlMap(request, jthdxsjl); //得到map //getMap(request, jthdxsjl);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(jthdxsjllist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, jthdxsjllist, jthdxsjlid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "jthdxsjl查询结果.xls"; //注意：要修改该文件名
		String listname = jthdxsjllist;
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		JthdxsjlForm jthdxsjlForm = null;
		String module = request.getParameter("module");
		//用于区分对两类id的查找
		if (!isNullOrSpace(request.getParameter("next"))) {
			request.setAttribute("next", request.getParameter("next"));
		...
		if (!isNullOrSpace(module)
			...
			JthdxsjlManager mgr = (JthdxsjlManager) getBean("jthdxsjlManager");
			Jthdxsjl jthdxsjl = mgr.getJthdxsjl(id); //按照id进行查询
			jthdxsjlForm = (JthdxsjlForm) convert(jthdxsjl);
			request.setAttribute(Constants.PID, jthdxsjl.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, jthdxsjlForm);
