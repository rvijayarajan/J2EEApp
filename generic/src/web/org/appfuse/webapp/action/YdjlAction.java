...
import org.appfuse.service.YdjlManager;
...
import org.appfuse.model.Ydjl;

import org.appfuse.webapp.form.YdjlForm;

/**
 * Action class to handle CRUD on a Ydjl object
 *
 * @struts.action name="ydjlForm" path="/ydjls" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="ydjlForm" path="/editYdjl" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="ydjlForm" path="/saveYdjl" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/ydjlForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/ydjlList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/ydjlFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/ydjlView.jsp"
 */
public final class YdjlAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		YdjlForm ydjlForm = (YdjlForm) form;

		resetYdjl(ydjlForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		YdjlManager mgr = (YdjlManager) getBean("ydjlManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, YdjlManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			YdjlManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetYdjl((YdjlForm) form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetYdjl((YdjlForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeYdjl(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.ydjl.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "ydjl.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		YdjlForm ydjlForm = new YdjlForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String ydjllist = Constants.YDJL_LIST;
		String ydjlid = Constants.YDJL_ID;
		return do_find(mapping, form, request, ydjllist, ydjlid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String ydjllist = Constants.YDJL_LIST;
		String ydjlid = Constants.YDJL_ID;
		return do_Search(mapping, request, ydjllist, ydjlid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		YdjlForm ydjlForm = (YdjlForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(ydjlForm.getId()) || ydjlForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetYdjl(ydjlForm); //将ydjlForm清空
				}
			} else {
				Ydjl ydjl; //按照id进行查询
				try {
					YdjlManager mgr = (YdjlManager) getBean("ydjlManager");
					ydjl = mgr.getYdjl(id);
					ydjlForm = (YdjlForm) convert(ydjl);
					request.setAttribute(Constants.PID, ydjl.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, ydjlForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String ydjllist, String ydjlid)
			throws BeansException {
		String type = getType(request);
		...
		YdjlForm ydjl;
		try {
			ydjl = (YdjlForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			ydjl = null;
		}
		...
		map = SearchMap.getYdjlMap(request, ydjl); //得到map
		...
		ValueList valueList = vlh.getValueList(ydjllist, info);

		vlHelper.backupAndSet(request, valueList, ydjllist, ydjlid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		ydjlForm.setXgr(getXgr(request));
		ydjlForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(ydjlForm.getXgsj()));//打印修改时间。

		YdjlManager mgr = (YdjlManager) getBean("ydjlManager");
		Ydjl ydjl = (Ydjl) convert(ydjlForm);
		...
		try {
			String id = mgr.saveYdjl(ydjl); //保存信息
			ydjlForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "ydjl.saved");
			...
			saveMsg(request, messages, "error.ydjl.saved");
			request.setAttribute("pid", ydjlForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetYdjl(YdjlForm ydjlForm) {
		Ydjl ydjl = new Ydjl();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(ydjlForm, ydjl);
		} catch (IllegalAccessException e1) {
			log.debug("resetYdjl 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetYdjl 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		YdjlForm ydjl = (YdjlForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getYdjlMap(request, ydjl); //得到map //getMap(request, ydjl);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(ydjllist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, ydjllist, ydjlid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "ydjl查询结果.xls"; //注意：要修改该文件名
		String listname = ydjllist;
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			YdjlManager mgr = (YdjlManager) getBean("ydjlManager");
			Ydjl ydjl = mgr.getYdjl(id); //按照id进行查询
			ydjlForm = (YdjlForm) convert(ydjl);
			request.setAttribute(Constants.PID, ydjl.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, ydjlForm);
