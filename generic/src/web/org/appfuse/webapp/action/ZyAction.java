...
public final class ZyAction extends BaseAction {
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetZy((ZyForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeZy(id);  //删除记录
			} catch (RuntimeException e) {
	...
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String zylist, String zyid) throws BeansException {
		...
		map = SearchMap.getZyMap(request, zy); //得到map
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			ZyManager mgr = (ZyManager) getBean("zyManager");
            try {
                getZy(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.zy.selected", id);
    ...
    throws Exception {
		...
		zyForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(zyForm.getXgsj()));//打印修改时间。
	...
	private void resetZy(ZyForm zyForm) {
		...
		try {
			...
			log.debug("resetZy 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetZy 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String zylist, String zyid) throws BeansException {
		...
		map = SearchMap.getZyMap(request, zy); //得到map //getMap(request, zy); //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "zy查询结果.xls"; //注意：要修改该文件名
