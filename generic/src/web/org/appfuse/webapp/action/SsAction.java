...
import org.appfuse.service.SsManager;
...
import org.appfuse.model.Ss;

import org.appfuse.webapp.form.SsForm;

/**
 * Action class to handle CRUD on a Ss object
 *
 * @struts.action name="ssForm" path="/sss" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="ssForm" path="/editSs" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="ssForm" path="/saveSs" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/ssForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/ssList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/ssFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/ssView.jsp"
 */
public final class SsAction extends BaseAction {

    public ActionForward cancel(ActionMapping mapping, ActionForm form,
                                HttpServletRequest request,
                                HttpServletResponse response)
    throws Exception {
	 SsForm ssForm = (SsForm) form;

	 resetSs(ssForm); //FormBean复位
    ...
    throws Exception {
		...
		SsManager mgr = (SsManager) getBean("ssManager");
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			...
			SsManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			SsManager mgr, ActionMessages messages,
			String type) throws Exception {
		HttpSession session = request.getSession(); //得到Session，从Session中查找是否有未处理的id
		String id = null;
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetSs((SsForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeSs(id);  //删除记录
			} catch (RuntimeException e) {
				getError(session, e, "error.ss.deleted", id);
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "ss.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String sslist = Constants.SS_LIST;
		String ssid = Constants.SS_ID;
		return do_find(mapping, form, request, sslist, ssid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String sslist = Constants.SS_LIST;
		String ssid = Constants.SS_ID;
		return do_Search(mapping, request, sslist, ssid);
	}
    ...
    throws Exception {
		...
		boolean isNew = ("".equals(ssForm.getId()) || ssForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			SsManager mgr = (SsManager) getBean("ssManager");
			 try {
                getSs(mapping, request, id, mgr);
            } catch (RuntimeException e) {
                getError(session, e, "error.ss.selected", id);
     ...
     * @throws Exception
     */
    private void getSs(ActionMapping mapping, HttpServletRequest request, String id, SsManager mgr) throws Exception {
        SsForm ssForm;
        Ss ss;
        ss = mgr.getSs(id);
        ssForm = (SsForm) convert(ss);
        updateFormBean(mapping, request, ssForm);
	 ...
	 */
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String sslist, String ssid) throws BeansException {
		String type = getType(request);
		...
		SsForm ss ;
		try {
			ss = (SsForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			 ss = null;
		}
		...
		map = SearchMap.getSsMap(request, ss); //得到map
		...
		ValueList valueList = vlh.getValueList(sslist, info);
		vlHelper.backupAndSet(request, valueList, sslist, ssid);
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			SsManager mgr = (SsManager) getBean("ssManager");
            try {
                getSs(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.ss.selected", id);
    ...
    throws Exception {
		...
		//得到修改人
		ssForm.setXgr(getXgr(request));
		ssForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(ssForm.getXgsj()));//打印修改时间。

		SsManager mgr = (SsManager) getBean("ssManager");
		Ss ss = (Ss) convert(ssForm);
		...
		try {
			id = mgr.saveSs(ss); //保存信息

			ssForm.setId(id);

			saveMsg(request, messages, "ss.saved");

			return mapping.findForward("view");
		} catch (RuntimeException e) {
			e.printStackTrace();
			saveMsg(request, messages, "error.ss.saved");
	...
	public ActionForward batch_ChangeID(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            getError(session, e1, "error.ss.selected", id);
        }
	 ...
	 * 
	 */
	private void resetSs(SsForm ssForm) {
		Ss ss = new Ss();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(ssForm, ss);
		} catch (IllegalAccessException e1) {
			log.debug("resetSs 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetSs 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	 ...
	 * @throws BeansException
	 */
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String sslist, String ssid) throws BeansException {
		HttpSession session = request.getSession();
		//清除Session中存有的信息
		session.removeAttribute(Constants.SEL_IDS);
		String type = getType(request);

		SsForm ss = (SsForm) form;
		//备份与字段相关的form信息
		session.setAttribute("form",ss);
		...
		map = SearchMap.getSsMap(request, ss); //得到map //getMap(request, ss); //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(sslist, new ValueListInfo(
				map));
		vlHelper.backupAndSet(request, valueList, sslist, ssid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "ss查询结果.xls"; //注意：要修改该文件名
		String listname = sslist;
