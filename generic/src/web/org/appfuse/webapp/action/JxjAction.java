...
import org.appfuse.service.JxjManager;
...
import org.appfuse.model.Jxj;

import org.appfuse.webapp.form.JxjForm;

/**
 * Action class to handle CRUD on a Jxj object
 *
 * @struts.action name="jxjForm" path="/jxjs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="jxjForm" path="/editJxj" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="jxjForm" path="/saveJxj" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/jxjForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/jxjList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/jxjFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/jxjView.jsp"
 */
public final class JxjAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JxjForm jxjForm = (JxjForm) form;

		resetJxj(jxjForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		JxjManager mgr = (JxjManager) getBean("jxjManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, JxjManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			JxjManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJxj((JxjForm) form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJxj((JxjForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeJxj(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.jxj.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "jxj.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String jxjlist = Constants.JXJ_LIST;
		String jxjid = Constants.JXJ_ID;
		return do_find(mapping, form, request, jxjlist, jxjid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String jxjlist = Constants.JXJ_LIST;
		String jxjid = Constants.JXJ_ID;
		return do_Search(mapping, request, jxjlist, jxjid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		JxjForm jxjForm = (JxjForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(jxjForm.getId()) || jxjForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					JxjManager mgr = (JxjManager) getBean("jxjManager");
					jxj = mgr.getJxj(id);
					jxjForm = (JxjForm) convert(jxj);
					request.setAttribute(Constants.PID, jxj.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, jxjForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String jxjlist, String jxjid)
			throws BeansException {
		String type = getType(request);
		...
		JxjForm jxj;
		try {
			jxj = (JxjForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			jxj = null;
		}
		...
		map = SearchMap.getJxjMap(request, jxj); //得到map
		...
		ValueList valueList = vlh.getValueList(jxjlist, info);

		vlHelper.backupAndSet(request, valueList, jxjlist, jxjid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		jxjForm.setXgr(getXgr(request));
		jxjForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(jxjForm.getXgsj()));//打印修改时间。

		JxjManager mgr = (JxjManager) getBean("jxjManager");
		Jxj jxj = (Jxj) convert(jxjForm);
		...
		try {
			String id = mgr.saveJxj(jxj); //保存信息
			jxjForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "jxj.saved");
			...
			saveMsg(request, messages, "error.jxj.saved");
			request.setAttribute("pid", jxjForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetJxj(JxjForm jxjForm) {
		Jxj jxj = new Jxj();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(jxjForm, jxj);
		} catch (IllegalAccessException e1) {
			log.debug("resetJxj 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetJxj 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		JxjForm jxj = (JxjForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getJxjMap(request, jxj); //得到map //getMap(request, jxj);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(jxjlist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, jxjlist, jxjid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "jxj查询结果.xls"; //注意：要修改该文件名
		String listname = jxjlist;
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			JxjManager mgr = (JxjManager) getBean("jxjManager");
			Jxj jxj = mgr.getJxj(id); //按照id进行查询
			jxjForm = (JxjForm) convert(jxj);
			request.setAttribute(Constants.PID, jxj.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, jxjForm);
