...
public final class LhjlAction extends BaseAction {
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetLhjl((LhjlForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetLhjl(lhjlForm); //将lhjlForm清空
				}
			} else {
				Lhjl lhjl; //按照id进行查询
				try {
					...
					request.setAttribute(Constants.PID, lhjl.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
	...
	public ActionForward do_Search(ActionMapping mapping,
		...
		map = SearchMap.getLhjlMap(request, lhjl); //得到map
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		lhjlForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(lhjlForm.getXgsj()));//打印修改时间。
		...
		try {
			String id = mgr.saveLhjl(lhjl); //保存信息
			lhjlForm.setId(id);
			//写入保存成功信息
			...
			request.setAttribute("pid", lhjlForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	...
	private void resetLhjl(LhjlForm lhjlForm) {
		...
		try {
			...
			log.debug("resetLhjl 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetLhjl 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		map = SearchMap.getLhjlMap(request, lhjl); //得到map //getMap(request, lhjl);
		// //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "lhjl查询结果.xls"; //注意：要修改该文件名
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			LhjlManager mgr = (LhjlManager) getBean("lhjlManager");
			Lhjl lhjl = mgr.getLhjl(id); //按照id进行查询
			lhjlForm = (LhjlForm) convert(lhjl);
			request.setAttribute(Constants.PID, lhjl.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, lhjlForm);
