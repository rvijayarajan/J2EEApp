...
import org.appfuse.service.TfsjManager;
...
import org.appfuse.model.Tfsj;

import org.appfuse.webapp.form.TfsjForm;

/**
 * Action class to handle CRUD on a Tfsj object
 *
 * @struts.action name="tfsjForm" path="/tfsjs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="tfsjForm" path="/editTfsj" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="tfsjForm" path="/saveTfsj" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/tfsjForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/tfsjList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/tfsjFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/tfsjView.jsp"
 */
public final class TfsjAction extends BaseAction {

    public ActionForward cancel(ActionMapping mapping, ActionForm form,
                                HttpServletRequest request,
                                HttpServletResponse response)
    throws Exception {
	 TfsjForm tfsjForm = (TfsjForm) form;

	 resetTfsj(tfsjForm); //FormBean复位
    ...
    throws Exception {
		...
		TfsjManager mgr = (TfsjManager) getBean("tfsjManager");
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			...
			TfsjManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetTfsj((TfsjForm)form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			TfsjManager mgr, ActionMessages messages,
			String type) throws Exception {
		HttpSession session = request.getSession(); //得到Session，从Session中查找是否有未处理的id
		String id = null;
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetTfsj((TfsjForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeTfsj(id);  //删除记录
			} catch (RuntimeException e) {
				getError(session, e, "error.tfsj.deleted", id);
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "tfsj.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		TfsjForm tfsjForm = new TfsjForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String tfsjlist = Constants.TFSJ_LIST;
		String tfsjid = Constants.TFSJ_ID;
		return do_find(mapping, form, request, tfsjlist, tfsjid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String tfsjlist = Constants.TFSJ_LIST;
		String tfsjid = Constants.TFSJ_ID;
		return do_Search(mapping, request, tfsjlist, tfsjid);
	}
    ...
    throws Exception {
		...
		boolean isNew = ("".equals(tfsjForm.getId()) || tfsjForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			TfsjManager mgr = (TfsjManager) getBean("tfsjManager");
			 try {
                getTfsj(mapping, request, id, mgr);
            } catch (RuntimeException e) {
                getError(session, e, "error.tfsj.selected", id);
     ...
     * @throws Exception
     */
    private void getTfsj(ActionMapping mapping, HttpServletRequest request, String id, TfsjManager mgr) throws Exception {
        TfsjForm tfsjForm;
        Tfsj tfsj;
        tfsj = mgr.getTfsj(id);
        tfsjForm = (TfsjForm) convert(tfsj);
        updateFormBean(mapping, request, tfsjForm);
	 ...
	 */
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String tfsjlist, String tfsjid) throws BeansException {
		String type = getType(request);
		...
		TfsjForm tfsj ;
		try {
			tfsj = (TfsjForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			 tfsj = null;
		}
		...
		map = SearchMap.getTfsjMap(request, tfsj); //得到map
		...
		ValueList valueList = vlh.getValueList(tfsjlist, info);
		vlHelper.backupAndSet(request, valueList, tfsjlist, tfsjid);
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			TfsjManager mgr = (TfsjManager) getBean("tfsjManager");
            try {
                getTfsj(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.tfsj.selected", id);
    ...
    throws Exception {
		...
		//得到修改人
		tfsjForm.setXgr(getXgr(request));
		tfsjForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(tfsjForm.getXgsj()));//打印修改时间。

		TfsjManager mgr = (TfsjManager) getBean("tfsjManager");
		Tfsj tfsj = (Tfsj) convert(tfsjForm);
		...
		try {
			id = mgr.saveTfsj(tfsj); //保存信息

			tfsjForm.setId(id);

			saveMsg(request, messages, "tfsj.saved");

			return mapping.findForward("view");
		} catch (RuntimeException e) {
			e.printStackTrace();
			saveMsg(request, messages, "error.tfsj.saved");
	...
	public ActionForward batch_ChangeID(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            getError(session, e1, "error.tfsj.selected", id);
        }
	 ...
	 * 
	 */
	private void resetTfsj(TfsjForm tfsjForm) {
		Tfsj tfsj = new Tfsj();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(tfsjForm, tfsj);
		} catch (IllegalAccessException e1) {
			log.debug("resetTfsj 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetTfsj 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	 ...
	 * @throws BeansException
	 */
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String tfsjlist, String tfsjid) throws BeansException {
		HttpSession session = request.getSession();
		//清除Session中存有的信息
		session.removeAttribute(Constants.SEL_IDS);
		String type = getType(request);

		TfsjForm tfsj = (TfsjForm) form;
		//备份与字段相关的form信息
		session.setAttribute("form",tfsj);
		...
		map = SearchMap.getTfsjMap(request, tfsj); //得到map //getMap(request, tfsj); //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(tfsjlist, new ValueListInfo(
				map));
		vlHelper.backupAndSet(request, valueList, tfsjlist, tfsjid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "tfsj查询结果.xls"; //注意：要修改该文件名
		String listname = tfsjlist;
