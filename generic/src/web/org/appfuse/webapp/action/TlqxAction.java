...
public final class TlqxAction extends BaseAction {
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {  
			resetTlqx((TlqxForm)form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			try{
				mgr.removeTlqx(id);  //删除记录
			} catch (RuntimeException e) {
	...
	public ActionForward do_Search(ActionMapping mapping, HttpServletRequest request, String tlqxlist, String tlqxid) throws BeansException {
		...
		map = SearchMap.getTlqxMap(request, tlqx); //得到map
	...
	public ActionForward edit_many(ActionMapping mapping, ActionForm form,
		...
		if (id == null) { //如果用户没选择id,同时浏览器关闭了javascript的功能
			...
			TlqxManager mgr = (TlqxManager) getBean("tlqxManager");
            try {
                getTlqx(mapping, request, id, mgr); //更新FormBean
            } catch (RuntimeException e) {
                getError(session, e, "error.tlqx.selected", id);
    ...
    throws Exception {
		...
		tlqxForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(tlqxForm.getXgsj()));//打印修改时间。
	...
	private void resetTlqx(TlqxForm tlqxForm) {
		...
		try {
			...
			log.debug("resetTlqx 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetTlqx 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form, HttpServletRequest request, String tlqxlist, String tlqxid) throws BeansException {
		...
		map = SearchMap.getTlqxMap(request, tlqx); //得到map //getMap(request, tlqx); //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "tlqx查询结果.xls"; //注意：要修改该文件名
