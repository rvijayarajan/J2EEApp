...
import org.appfuse.service.JkManager;
...
import org.appfuse.model.Jk;

import org.appfuse.webapp.form.JkForm;

/**
 * Action class to handle CRUD on a Jk object
 *
 * @struts.action name="jkForm" path="/jks" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="jkForm" path="/editJk" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="jkForm" path="/saveJk" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/jkForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/jkList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/jkFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/jkView.jsp"
 */
public final class JkAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JkForm jkForm = (JkForm) form;

		resetJk(jkForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		JkManager mgr = (JkManager) getBean("jkManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, JkManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			JkManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJk((JkForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeJk(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.jk.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "jk.deleted");
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String jklist = Constants.JK_LIST;
		String jkid = Constants.JK_ID;
		return do_find(mapping, form, request, jklist, jkid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String jklist = Constants.JK_LIST;
		String jkid = Constants.JK_ID;
		return do_Search(mapping, request, jklist, jkid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		JkForm jkForm = (JkForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(jkForm.getId()) || jkForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					JkManager mgr = (JkManager) getBean("jkManager");
					jk = mgr.getJk(id);
					jkForm = (JkForm) convert(jk);
					request.setAttribute(Constants.PID, jk.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, jkForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String jklist, String jkid)
			throws BeansException {
		String type = getType(request);
		...
		JkForm jk;
		try {
			jk = (JkForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			jk = null;
		}
		...
		map = SearchMap.getJkMap(request, jk); //得到map
		...
		ValueList valueList = vlh.getValueList(jklist, info);

		vlHelper.backupAndSet(request, valueList, jklist, jkid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		jkForm.setXgr(getXgr(request));
		jkForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(jkForm.getXgsj()));//打印修改时间。

		JkManager mgr = (JkManager) getBean("jkManager");
		Jk jk = (Jk) convert(jkForm);
		...
		try {
			String id = mgr.saveJk(jk); //保存信息
			jkForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "jk.saved");
			...
			saveMsg(request, messages, "error.jk.saved");
			request.setAttribute("pid", jkForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetJk(JkForm jkForm) {
		Jk jk = new Jk();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(jkForm, jk);
		} catch (IllegalAccessException e1) {
			log.debug("resetJk 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetJk 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		JkForm jk = (JkForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getJkMap(request, jk); //得到map //getMap(request, jk);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(jklist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, jklist, jkid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "jk查询结果.xls"; //注意：要修改该文件名
		String listname = jklist;
