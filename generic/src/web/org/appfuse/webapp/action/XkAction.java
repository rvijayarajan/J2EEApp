...
public final class XkAction extends BaseAction {
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetXk((XkForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			id = request.getParameter("id");
			if (!isNullOrSpace(id)) {
				mgr.removeXk(id); //删除记录
			}
		}
	...
	private ActionForward delEditOrView(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, ActionMessages messages, HttpSession session, String id) throws Exception {
		String[] sel_ids = (String[]) session
				.getAttribute(Constants.SEL_IDS); //获取Session中的批量选中的id的数组
		//如果next = "f",则是从健康list中过来的,否则，从jbxxlist过来的
		...
		if (pos >= sel_ids.length - 1) { //如果为最后一条记录
			if (sel_ids.length > 1) {
				...
				decPos(session, pos); //将pos减1
				return goToEdit(mapping, form, request, response, messages, "xk"); //调转到Search方法
			}
		}
		//获取删除后的上一条、下一条的图标
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				request.setAttribute(Constants.PID, id); //将当前的pid放入request中
				String from = request.getParameter("from");
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetXk(xkForm); //将xkForm清空
				}
			} else {
				Xk xk; //按照id进行查询
				try {
					...
					request.setAttribute(Constants.PID, xk.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
	...
	public ActionForward do_Search(ActionMapping mapping,
		...
		map = SearchMap.getXkMap(request, xk); //得到map //getMap(request, xk);
											   // //得到map
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		xkForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(xkForm.getXgsj()));//打印修改时间。
		...
		try {
			...
			messages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
					"error.xk.saved"));
			saveMessages(session, messages);
			request.setAttribute("pid", xkForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	...
	private void resetXk(XkForm xkForm) {
		...
		try {
			...
			log.debug("resetXk 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetXk 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		vlHelper.backupAndSet(request, valueList, xklist, xkid);
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "xk查询结果.xls"; //注意：要修改该文件名
