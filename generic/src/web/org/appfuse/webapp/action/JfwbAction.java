...
import org.appfuse.service.JfwbManager;
...
import org.appfuse.model.Jfwb;

import org.appfuse.webapp.form.JfwbForm;

/**
 * Action class to handle CRUD on a Jfwb object
 *
 * @struts.action name="jfwbForm" path="/jfwbs" scope="request"
 *  validate="false" parameter="method" input="mainMenu"
 * @struts.action name="jfwbForm" path="/editJfwb" scope="request"
 *  validate="false" parameter="method" input="list"
 * @struts.action name="jfwbForm" path="/saveJfwb" scope="request"
 *  validate="true" parameter="method" input="edit"
 * 
 * @struts.action-forward name="edit" path="/WEB-INF/pages/jfwbForm.jsp"
 * @struts.action-forward name="list" path="/WEB-INF/pages/jfwbList.jsp"
 * @struts.action-forward name="find" path="/WEB-INF/pages/jfwbFind.jsp"
 * @struts.action-forward name="view" path="/WEB-INF/pages/jfwbView.jsp"
 */
public final class JfwbAction extends BaseAction {
   	public ActionForward cancel(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JfwbForm jfwbForm = (JfwbForm) form;

		resetJfwb(jfwbForm); //FormBean复位
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		JfwbManager mgr = (JfwbManager) getBean("jfwbManager");
	 ...
	 * @throws Exception
	 */
	private ActionForward delReturn(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response, JfwbManager mgr, ActionMessages messages) throws Exception {
	...
	private ActionForward delMany(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response,
			JfwbManager mgr, ActionMessages messages, String type)
			throws Exception {
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJfwb((JfwbForm) form);
			saveToken(request);
		} else {
			resetToken(request);
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetJfwb((JfwbForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
			...
			if (!isNullOrSpace(id)) {
				try {
					mgr.removeJfwb(id); //删除记录
				} catch (RuntimeException e) {
					e.printStackTrace();
					getError(session, e, "error.jfwb.deleted", id);
				}
			}
		}

		if (Constants.DEL.equals(type)) { //如果是删除普通的单条记录,跳转到查询结果页面
			saveMsg(request, messages, "jfwb.deleted");
	...
	public ActionForward gotoFind(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JfwbForm jfwbForm = new JfwbForm();
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String jfwblist = Constants.JFWB_LIST;
		String jfwbid = Constants.JFWB_ID;
		return do_find(mapping, form, request, jfwblist, jfwbid);
	}
	...
	public ActionForward search(ActionMapping mapping, ActionForm form,
		...
		String jfwblist = Constants.JFWB_LIST;
		String jfwbid = Constants.JFWB_ID;
		return do_Search(mapping, request, jfwblist, jfwbid);
	}
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,
		...
		JfwbForm jfwbForm = (JfwbForm) form;
		//判断是新建还是修改
		boolean isNew = ("".equals(jfwbForm.getId()) || jfwbForm.getId() == null);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				if (!isNullOrSpace(from) && from.equals("del_many_one")) {
					resetJfwb(jfwbForm); //将jfwbForm清空
				}
			} else {
				Jfwb jfwb; //按照id进行查询
				try {
					JfwbManager mgr = (JfwbManager) getBean("jfwbManager");
					jfwb = mgr.getJfwb(id);
					jfwbForm = (JfwbForm) convert(jfwb);
					request.setAttribute(Constants.PID, jfwb.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
			...
			updateFormBean(mapping, request, jfwbForm);
	...
	public ActionForward do_Search(ActionMapping mapping,
			HttpServletRequest request, String jfwblist, String jfwbid)
			throws BeansException {
		String type = getType(request);
		...
		JfwbForm jfwb;
		try {
			jfwb = (JfwbForm) session.getAttribute("form");
		} catch (RuntimeException e) {
			e.printStackTrace();
			jfwb = null;
		}
		...
		map = SearchMap.getJfwbMap(request, jfwb); //得到map
		...
		ValueList valueList = vlh.getValueList(jfwblist, info);

		vlHelper.backupAndSet(request, valueList, jfwblist, jfwbid);
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		//得到修改人和修改时间
		jfwbForm.setXgr(getXgr(request));
		jfwbForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(jfwbForm.getXgsj()));//打印修改时间。

		JfwbManager mgr = (JfwbManager) getBean("jfwbManager");
		Jfwb jfwb = (Jfwb) convert(jfwbForm);
		...
		try {
			String id = mgr.saveJfwb(jfwb); //保存信息
			jfwbForm.setId(id);
			//写入保存成功信息
			saveMsg(request, messages, "jfwb.saved");
			...
			saveMsg(request, messages, "error.jfwb.saved");
			request.setAttribute("pid", jfwbForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	 ...
	 *  
	 */
	private void resetJfwb(JfwbForm jfwbForm) {
		Jfwb jfwb = new Jfwb();
		//此处适用了Apache的BeanUtils类的copyProperties方法,该方法第一个参数是目标对象，后面的参数是源对象
		try {
			BeanUtils.copyProperties(jfwbForm, jfwb);
		} catch (IllegalAccessException e1) {
			log.debug("resetJfwb 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetJfwb 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		JfwbForm jfwb = (JfwbForm) form;

		ValueListHandler vlh = getValueListHandler();

		Map map = new HashMap();

		map = SearchMap.getJfwbMap(request, jfwb); //得到map //getMap(request, jfwb);
		// //得到条件Map

		ValueListHandlerHelper vlHelper = getValueListHelper();
		ValueList valueList = vlh.getValueList(jfwblist, new ValueListInfo(map));
		vlHelper.backupAndSet(request, valueList, jfwblist, jfwbid);

		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "jfwb查询结果.xls"; //注意：要修改该文件名
		String listname = jfwblist;
	...
	private void getNext(ActionMapping mapping, HttpServletRequest request,
		...
		if (!isNullOrSpace(module)
			...
			JfwbManager mgr = (JfwbManager) getBean("jfwbManager");
			Jfwb jfwb = mgr.getJfwb(id); //按照id进行查询
			jfwbForm = (JfwbForm) convert(jfwb);
			request.setAttribute(Constants.PID, jfwb.getJbxxid()); //将当前的pid放入request中
		}
		updateFormBean(mapping, request, jfwbForm);
