...
public final class ZsAction extends BaseAction {
	...
	private ActionForward delOne(ActionMapping mapping, ActionForm form,
		...
		//防止重复提交
		if (!isTokenValid(request)) {
			resetZs((ZsForm) form);//如果已经执行过了,则跳入本部分
			saveToken(request);
		} else {
			resetToken(request);
		...
		if (isNew) { //当选择新建时，删除图标为灰色，功能不可使用
			...
			if (!isNullOrSpace(module)
				...
				try {
					...
					request.setAttribute(Constants.PID, zs.getJbxxid()); //将当前的pid放入request中
				} catch (Exception e) {
					e.printStackTrace();
					return search(mapping, form, request, response);
				}
	...
	public ActionForward do_Search(ActionMapping mapping,
		...
		map = SearchMap.getZsMap(request, zs); //得到map
	...
	public ActionForward save(ActionMapping mapping, ActionForm form,
		...
		zsForm.setXgsj(NowDate.getNowStrIsNullOrEmpty(zsForm.getXgsj()));//打印修改时间。
		...
		try {
			...
			request.setAttribute("pid", zsForm.getJbxxid()); //将当前的id写入request的Attribute中
			return mapping.findForward("edit");
		}
	}
	...
	private void resetZs(ZsForm zsForm) {
		...
		try {
			...
			log.debug("resetZs 出错-IllegalAccessException");
			e1.printStackTrace();
		} catch (InvocationTargetException e1) {
			log.debug("resetZs 出错-InvocationTargetException");
			e1.printStackTrace();
		}
	}
	...
	public ActionForward do_find(ActionMapping mapping, ActionForm form,
		...
		map = SearchMap.getZsMap(request, zs); //得到map //getMap(request, zs);
		// //得到条件Map
		...
		//导出特定参数,从而完成Excel文件
		String excelFileName = realPath + "zs查询结果.xls"; //注意：要修改该文件名
