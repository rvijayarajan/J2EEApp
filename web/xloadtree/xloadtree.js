...
function _xmlTreeToJsTree(oNode) {
	...
	var jsNode;
	if (src != null && src != "")
	//change_by liuyan 判断是否到最后1级，看是否含有 '&key=rdev&',有则到最后1级
		if (src.indexOf('&key=rdev&')==-1)
		{
...
function _xmlFileLoaded(oXmlDoc, jsParentNode) {
	...
	//	alert(oXmlDoc.xml);
		...
		//change_by liuyan 根据现在返回的xml文件格式 制定相关的解析结构
		// there is one extra level of tree elements
		var root = oXmlDoc.documentElement;
		// loop through all tree children
		var cs = root.childNodes;
		//var cs=root.getElementsByTagName("tree") ;
		if (root.tagName=="root")
		{
        cs=oXmlDoc.documentElement.selectNodes("service/tree/tree");
		}
		var l = cs.length;
		//alert(0+""+cs[0].tagName);
		for (var i = 0; i < l; i++) {
			//alert(cs[0].getAttribute("src") );
			if (cs[i].tagName == "tree") {
				bAnyChildren = true;
				bIndent = true;
