...
//	email :	fuushikaden@yahoo.com
	...
	var gotoString = "跳转到当前月份"
	var todayString = "今天 是 "
	var weekString = "周"
	var xq = "星期"
	var scrollLeftMessage = "单击鼠标左键，选择上一个月，按住左键不动，自动向前滚动"
	var scrollRightMessage = "单击鼠标左键，选择下一个月，按住左键不动，自动向后滚动"
	var selectMonthMessage = "单击鼠标左键，选择月份"
	var selectYearMessage = "单击鼠标左键，选择年份"
   ...
   function WeekNbr(n) {
	...
	function popUpCalendar(ctl,	ctl2, format) {
		...
		//李铁峰修改的部分
		//目的：这样修改是为了调用方法可以是这样的－popUpCalendar(this, document.forms[0].datevalue)，不用输入第三个变量的值，
		if ((format == null) || (format == "") || (format == "undefied"))
		{
			format = "yyyy-mm-dd";
		}
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				if (leftpos<=-320)
				{
					crossobj.left =	fixedX==-1 ? ctl.offsetLeft	+ leftpos + 255 :	fixedX;
				}
				else
				{
				...
				if(toppos<=-25)
				{
					crossobj.top = fixedY==-1 ?	ctl.offsetTop +	toppos + ctl.offsetHeight +	2 + 200:	fixedY
				}
				else
				{
