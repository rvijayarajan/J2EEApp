...
public final class LoginServlet extends HttpServlet {
    ...
    throws IOException, ServletException {
        ...
        if (request.getRemoteUser() != null) {
            if (log.isDebugEnabled()) {
                log.debug("User '" + request.getRemoteUser() +
                          "' already logged in, routing to mainMenu/frameset");
            }

            response.sendRedirect(request.getContextPath() + "/frameset.jsp");
        ...
        //      将登录用户名放入session中，以便session过期清除相应标记
        ...
        if (redirectString == null) {
			 ...
			 * 删除自己使用的临时目录
			 */
			String selfPath = BaseAction.getPath(request.getSession().getServletContext().getRealPath("/") 
                    + username);
			// request.getSession().getServletContext()
			// .getRealPath("/") + username+"//";
			if (username!=null&&username.trim().length()>0)
				BaseAction.deleteDir(selfPath);
			// --------end of szy add ------------
...
/*			if (username != null || username.length() > 0) {
				...
				SysUserManager mgr = (SysUserManager) ctx.getBean("sysUserManager");
				SysUser user = mgr.getSysUser(username);
				RoleAppManager mgrRole = (RoleAppManager)ctx.getBean("roleAppManager");
				RoleApp role = mgrRole.getRoleApp(user.getRoleApp().getRolename());
				Set<OpRegister> ops =(Set<OpRegister>) role.getOpregisters();
 ...
 * @return
 */
	private boolean isNullOrSpace(String loginmark) {
		if (loginmark != null && !loginmark.trim().equals("") && !loginmark.trim().equals("null")) {
			return false;
		} else
			return true;
		}
